...
async function updateBackendWithUserBalace(userId, balance) {
    const backendApiBaseUrl = process.env.BACKEND_API_BASE_URL;
    if (!backendApiBaseUrl) {
    ...
    try {
        ...
        return responseData; // Return whatever the backend responds with
    } catch (error) {
...
app.post('/update-backend-with-user-balance', async (req, res) => {
    ...
    try {
        await lock.acquire(userId, async () => {
            ...
            const updateResponseFromBackend = await updateBackendWithUserBalace(userId, currentBalance);
