...
export const setCountersValues = (credits, bet, win, freeGamesNum, freeGamesSum, freeGamesBank) => {
    if (!freeGamesSum) {
...
export const drawWinningLinesList = (div, winningLines, winningScatters) => {
...
export const drawOutcome = (reelsSymbols, bet, credits, freeGamesNum, freeGamesSum, freeGamesBank, winningLines, winningScatters) => {
    const reelsTable = document.getElementById("reels");
...
export const drawReelsSymbols = (reelsSymbols, table) => {
...
export const play = async () => {
    ...
    drawOutcome(data.reelsSymbols, data.bet, data.credits, data.freeGamesNum, data.freeGamesSum, data.freeGamesBank, data.winningLines, data.winningScatters);
};
export const getAnyWin = async () => {
    const data = (await getAnyWinData());
    drawOutcome(data.reelsSymbols, data.bet, data.credits, data.freeGamesNum, data.freeGamesSum, data.freeGamesBank, data.winningLines, data.winningScatters);
};
export const getSymbolWin = async (itemId, times) => {
    const data = (await getSymbolWinData(itemId, times));
    drawOutcome(data.reelsSymbols, data.bet, data.credits, data.freeGamesNum, data.freeGamesSum, data.freeGamesBank, data.winningLines, data.winningScatters);
};
export const getCustomScenario = async (scenarioId) => {
    const data = (await getCustomScenarioData(scenarioId));
    drawOutcome(data.reelsSymbols, data.bet, data.credits, data.freeGamesNum, data.freeGamesSum, data.freeGamesBank, data.winningLines, data.winningScatters);
};
