...
*/
import { PlayFreeGamesStrategy, SimulationConfig, SymbolsCombinationsGenerator, VideoSlotWithFreeGamesSessionSerializer, } from "pokie";
...
linesAndScattersConfig.setPlayStrategy({
    canPlayNextSimulationRound: (session) => !(Object.keys(session.getWinningLines()).length > 0 && Object.keys(session.getWinningScatters()).length > 0),
});
